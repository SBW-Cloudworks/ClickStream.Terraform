...
variable "enable_nat_gateway" {
  description = "Whether to provision a NAT Gateway for private subnet egress (default off per design)."
  type        = bool
  default     = false
}

variable "allowed_admin_cidrs" {
  description = "CIDR blocks allowed to reach admin endpoints (Shiny via ALB/NLB)."
  type        = list(string)
  default     = []
}
...
variable "analytics_instance_type" {
  description = "Instance type for the combined DWH + Shiny EC2."
  type        = string
  default     = "t3.large"
}
...
variable "analytics_ami_id" {
  description = "AMI ID override for DWH + Shiny EC2."
  type        = string
  default     = ""
}
...
variable "analytics_key_name" {
  description = "SSH key pair name for DWH + Shiny EC2."
  type        = string
  default     = ""
}
...
variable "analytics_root_volume_gb" {
  description = "Root volume size for DWH + Shiny EC2."
  type        = number
  default     = 200
}
...
variable "shiny_user_data" {
  description = "User data script for DWH + Shiny EC2."
  type        = string
  default     = <<-EOT
    #!/bin/bash
    echo "Placeholder user-data for DWH+Shiny instance. Replace with Postgres/R/Shiny install steps." > /var/log/analytics-user-data.log
  EOT
}
